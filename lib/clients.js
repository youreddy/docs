...
function decryptSensitiveFields (client) {
  ...
  sensitiveFields.forEach(function (f) {
    if (client[f]) {
      client[f] = utils.decryptAesSha256(key, client[f], true);
    }
  });

  if (client.signingKey && client.signingKey.key) {
    client.signingKey.key = utils.decryptAesSha256(key, client.signingKey.key, true);
  }
